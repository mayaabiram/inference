...
def process(i):
    try:
        ...
        try:
            ...
            asdas = tok(explanation)["input_ids"]
            if len(asdas)>500:
                print('Line is too long, skipping just in case...')
                return asses
            ...
            if "___" in event:
                ...
                # print(causal_event)
                asdas = tok(causal_event)["input_ids"]
                if len(asdas) > 500:
                    print('Line is too long, skipping just in case...')
                    pass
                else:
...
if __name__ == '__main__':
    ...
    for x in [i % cuda_count for i in range(process_num)]:
        q.put(x)
    tottqdm = int(len(mydata.index)/args.cuda_num)
    with Pool(process_num = 2,initargs=[mydata,tottqdm,q,howto]) as p:
        # final_data = p.map(process,data.index) #list of lists that may be null
        for l in p.imap_unordered(process,mydata.index):
